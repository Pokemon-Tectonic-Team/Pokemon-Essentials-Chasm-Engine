...
class PokeBattle_Move
    ...
    def calculateDamageForHitAI(user,target,type,baseDmg,numTargets,ai_context=nil)
        calculateDamageForHit(user,target,type,baseDmg,numTargets,true,ai_context)
    end
    ...
    def calculateDamageForHit(user,target,type,baseDmg,numTargets,aiCheck=false,ai_context=nil)
        ...
        pbCalcDamageMultipliers(user,target,numTargets,type,baseDmg,multipliers,aiCheck,ai_context)
    ...
    def pbCalcDamageMultipliers(user,target,numTargets,type,baseDmg,multipliers,aiCheck=false,ai_context=nil)
        ...
        user.eachItemShouldApply(aiCheck) do |item|
            BattleHandlers.triggerDamageCalcUserItem(item,user,target,self,multipliers,baseDmg,type,aiCheck,ai_context)
        end
