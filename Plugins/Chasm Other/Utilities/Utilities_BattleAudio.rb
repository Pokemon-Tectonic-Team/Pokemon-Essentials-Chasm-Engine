...
def pbGetTrainerBattleBGM(trainer) # can be a Player, NPCTrainer or an array of them
    ...
    trainerarray.each do |t|
        ...
        music = trainer_type_data.cursed_battle_BGM if $PokemonGlobal.tarot_amulet_active && trainer_type_data.cursed_battle_BGM
    end
    ret = pbStringToAudioFile(music) if music && music != ""
    unless ret
        # Check map metadata
...
def pbGetTrainerBattleBGMFromType(trainertype)
    ...
    ret = trainer_type_data.cursed_battle_BGM if $PokemonGlobal.tarot_amulet_active && trainer_type_data.cursed_battle_BGM
    unless ret
        # Check map metadata
        map_metadata = GameData::MapMetadata.try_get($game_map.map_id)
        music = map_metadata ? map_metadata.trainer_battle_BGM : nil
