    ...
    class Avatar
        ...
        def initialize(hash)
            ...
            @abilities.each do |abilityID|
                ...
                Compiler.logLegalityError _INTL("Cut ability #{abilityID} is assigned to avatar #{@id}!")
            end

            [@moves1, @moves2, @moves3, @moves4, @moves5].flatten.each do |moveID|
                moveData = GameData::Move.get(moveID)
                next unless moveData.cut
                Compiler.logLegalityError _INTL("Cut move #{moveID} is assigned to avatar #{@id}!")
            end
        end
